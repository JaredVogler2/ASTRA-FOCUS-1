...
def get_late_parts_analysis():
    ...
    # Check if in production mode (using pre-computed schedules)
    if hasattr(current_app, 'current_schedule_data') and current_app.current_schedule_data and not scheduler:
        # Return placeholder data for production mode
        return jsonify({
            'message': 'Late parts analysis requires real-time scheduler. Using pre-computed schedules.',
            'late_parts': [],
