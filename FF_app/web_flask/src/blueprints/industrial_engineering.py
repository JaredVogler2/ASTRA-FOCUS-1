...
# Use absolute path relative to project root for consistent file location
from src.paths import IE_QUEUE_FILE as _IE_QUEUE_PATH
IE_QUEUE_FILE = str(_IE_QUEUE_PATH)
...
def flag_task_for_review():
    ...
    if any(item.get('task_id') == task_id and item.get('mechanic_name') == mechanic_name and item.get('reason') == reason and item.get('status') == 'open' for item in queue):
        current_app.logger.info(f"Duplicate feedback for task {task_id} by {mechanic_name} - skipping")
        existing = next(item for item in queue if item.get('task_id') == task_id and item.get('mechanic_name') == mechanic_name and item.get('reason') == reason and item.get('status') == 'open')
        return jsonify({
            'success': True,
            'message': f'Task {task_id} already flagged for IE review.',
